...
\evidencia{Creaci\'on de una clave de API en el panel de OpenAI (\texttt{platform.openai.com/api-keys}).}{capturas_openai/openai_01_api_keys.png}
\evidencia{Configuraci\'on de \texttt{OPENAI\_API\_KEY} en el archivo \texttt{.env} del backend.}{capturas_openai/openai_02_env.png}
...
\evidencia{Instancia EC2 (\texttt{FinZen App}) en estado \emph{Running} con IP el\'astica \texttt{54.198.140.159}.}{capturas_aws/aws_01_instancia.png}
\evidencia{Security Group con las reglas de entrada \texttt{22} (SSH) y \texttt{80} (HTTP).}{capturas_aws/aws_02_security_group.png}
\evidencia{Contenedores en ejecuci\'on (\texttt{docker compose ps} y \texttt{docker ps}) y prueba de las APIs v\'ia Nginx.}{capturas_aws/aws_03_contenedores.png}
\evidencia{Aplicaci\'on funcionando por la IP el\'astica (\texttt{http://54.198.140.159}).}{capturas_aws/aws_04_app.png}
\evidencia{Liberaci\'on del puerto \texttt{80}: Apache detenido y deshabilitado en la EC2.}{capturas_aws/aws_05_apache.png}
