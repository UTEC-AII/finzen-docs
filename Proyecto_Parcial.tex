...
\usepackage{titlesec}
% Encabezados de tamaño moderado y uniforme
\titleformat{\part}{\normalfont\Large\bfseries\centering}{}{0pt}{}
\titlespacing*{\part}{0pt}{0pt}{0.6\baselineskip}
...
\begin{itemize}[leftmargin=1.2cm]
  \item \textbf{Empresa:} FinZen Digital (empresa ficticia).
...
\begin{tabular}{>{\raggedright\arraybackslash}p{2.8cm} p{3.4cm} p{4.6cm} p{4.5cm}}
\toprule
\textbf{Producto} & \textbf{Tipos de clientes} & \textbf{Funcionalidades} & \textbf{Propuesta de valor} \\
\midrule
\textbf{PocketGuard} & Personas con presupuesto ajustado que buscan evitar sobregiros & C\'alculo del saldo disponible tras gastos fijos; l\'imites por categor\'ia; v\'inculo bancario; alertas & Simplicidad: indica cu\'anto se puede gastar hoy sin sobregirarse \\
\textbf{Copilot Money} & Usuarios de Apple (iOS/macOS) con ingresos medios y altos & Categorizaci\'on autom\'atica con ML; detecci\'on de suscripciones; presupuestos adaptativos; asistente conversacional & IA que aprende del usuario y reduce la categorizaci\'on manual \\
\textbf{Monarch Money} & Individuos y parejas que consolidan sus finanzas & V\'inculo a m\'as de 10\,000 instituciones; patrimonio neto; metas compartidas; asistente de IA & Gesti\'on integral y colaborativa en una sola aplicaci\'on \\
\textbf{YNAB} & Personas que adoptan un m\'etodo de presupuesto por sobres & Presupuesto base cero; asignaci\'on por categor\'ias; metas de ahorro; reportes; sincronizaci\'on bancaria & Disciplina financiera: asignar un destino a cada unidad monetaria \\
\midrule
\textbf{FinZen} (propuesto) & Personas y freelancers en Latinoam\'erica con movimientos en varias monedas & Registro manual multi-moneda; panel de control; asistente RAG en lenguaje natural & An\'alisis en lenguaje natural sin vincular cuentas bancarias ni compartir credenciales \\
\bottomrule
\end{tabular}
\caption{Cuadro comparativo del benchmark (competidores y FinZen).}
\end{table}

\textbf{Lectura del benchmark.} El mercado converge en dos ejes: la
\textbf{simplicidad} (PocketGuard) y la \textbf{automatizaci\'on con IA} (Copilot Money,
Monarch Money y YNAB). Sin embargo, todos comparten tres limitaciones que dejan un
espacio sin cubrir:
\begin{itemize}[leftmargin=1.2cm]
  \item \textbf{Dependen de vincular cuentas bancarias:} sin esa conexi\'on su IA no
        funciona, y muchos usuarios no quieren compartir sus credenciales.
  \item \textbf{Explican poco:} ofrecen cifras (cu\'anto se puede gastar o cu\'anto se
        gast\'o), pero no responden preguntas abiertas en lenguaje natural.
  \item \textbf{Asumen una sola moneda y un solo pa\'is:} no est\'an pensadas para
        usuarios que mueven dinero en varias monedas, algo frecuente en Latinoam\'erica.
\end{itemize}

\textbf{Oportunidad para FinZen.} Esa combinaci\'on ---lenguaje natural, sin vincular
cuentas y multi-moneda--- es precisamente el espacio sin cubrir. El benchmark no solo
valida la idea: muestra \emph{d\'onde} est\'a el diferenciador.
...
\textbf{El problema.} Entender las finanzas personales sigue siendo tedioso: las
aplicaciones l\'ideres exigen vincular cuentas bancarias, adoptar un m\'etodo de
presupuesto o interpretar gr\'aficos. Para quien maneja varias monedas, ninguna
responde a la pregunta m\'as simple: \emph{«¿en qu\'e se me va el dinero?»}.

\textbf{La propuesta.} FinZen convierte los movimientos del usuario en
\textbf{respuestas claras y conversacionales, en espa\~nol y en lenguaje natural}. El
usuario registra sus ingresos y gastos (en cualquier moneda) y le pregunta al
asistente; este responde con base en \textbf{sus propios datos}, citando los movimientos
que sustentan la respuesta.

\textbf{Diferenciadores frente a la competencia:}
\begin{itemize}[leftmargin=1.2cm]
  \item \textbf{Sin fricci\'on y sin riesgo:} no vincula cuentas bancarias ni solicita
        credenciales.
  \item \textbf{IA que responde, no que solo grafica:} recuperaci\'on sem\'antica
        (\texttt{text-embedding-3-large}) y generaci\'on aumentada (RAG) con citas y
        \emph{guardrails} anti-alucinaci\'on.
  \item \textbf{Pensada para Latinoam\'erica:} multi-moneda (PEN, USD, \dots) y zona
        horaria configurable.
  \item \textbf{Costo casi nulo:} el consumo de IA ronda $\sim$\$0.04 al mes para el
        volumen del proyecto.
\end{itemize}

\textbf{En una frase:} FinZen es \emph{``preg\'untale a tus finanzas''}: la forma m\'as
r\'apida de entender tu dinero, sin compartir tus cuentas.
...
\begingroup\footnotesize\setlength{\tabcolsep}{4pt}\renewcommand{\arraystretch}{1.35}
\begin{longtable}{p{1.5cm} p{1.75cm} p{7.15cm} c p{3.3cm}}
\toprule
...
\subsection{Configuraci\'on de la clave de OpenAI}
El asistente (HU7) usa la API de OpenAI para los \emph{embeddings} y el modelo de chat.
La clave se obtiene en el panel de OpenAI (\url{https://platform.openai.com/api-keys}):
\emph{API keys} $\rightarrow$ \emph{Create new secret key}. Luego se configura de una de
estas dos formas:

\begin{itemize}[leftmargin=1.2cm]
  \item \textbf{En el despliegue:} como variable \texttt{OPENAI\_API\_KEY} en el archivo
        \texttt{.env} del backend.
  \item \textbf{En tiempo de ejecuci\'on:} desde la interfaz web (\emph{Mi Perfil}), que la
        guarda en la base SQLite del \texttt{ai-service} (nunca en el navegador).
\end{itemize}

Si no se configura, el \texttt{ai-service} funciona en \textbf{modo local} (embeddings
deterministas) para poder probar sin conexi\'on.

\evidencia{Panel de OpenAI (\texttt{platform.openai.com/api-keys}): creaci\'on de una nueva clave secreta de API.}{capturas_openai/openai_01_api_keys.png}
\evidencia{Archivo \texttt{.env} del backend en la EC2, con \texttt{OPENAI\_API\_KEY}, \texttt{SECRET\_KEY} y \texttt{ALLOWED\_ORIGINS} configurados.}{capturas_openai/openai_02_env.png}
...
\evidencia{Petici\'on de registro: \texttt{POST /api/users/users} con el cuerpo JSON y respuesta \texttt{201 Created} con el usuario creado.}{capturas_postman/01_registrar_usuario.png}
\evidencia{Petici\'on de login: \texttt{POST /api/users/auth/login} que devuelve \texttt{200 OK} con el \texttt{access\_token} (JWT).}{capturas_postman/02_login.png}
\evidencia{Registro de un ingreso: \texttt{POST /api/incomes/incomes} con respuesta \texttt{201 Created}.}{capturas_postman/03_registrar_ingreso.png}
\evidencia{Registro de un gasto en soles (PEN): \texttt{POST /api/expenses/expenses} con respuesta \texttt{201 Created}.}{capturas_postman/04_registrar_gasto_pen_.png}
\evidencia{Registro de un gasto en d\'olares (USD): misma ruta, respuesta \texttt{201 Created}.}{capturas_postman/05_registrar_gasto_usd_.png}
\evidencia{Listado de gastos filtrado por categor\'ia: \texttt{GET /api/expenses/expenses/\{user\_id\}?category=Alimentacion} con respuesta \texttt{200 OK}.}{capturas_postman/06_listar_gastos_filtro_.png}
\evidencia{Consulta al asistente 1: ``¿en qu\'e categor\'ia gasto m\'as?'' (\texttt{POST /api/ai/query}) con respuesta en lenguaje natural.}{capturas_postman/07_consulta_ia_1.png}
\evidencia{Consulta al asistente 2: ``¿cu\'anto he gastado este mes?'' con la suma de los gastos por moneda.}{capturas_postman/08_consulta_ia_2.png}
\evidencia{Consulta al asistente 3: ``¿cu\'anto he recibido de ingresos?'' con el total de ingresos.}{capturas_postman/09_consulta_ia_3.png}
\evidencia{Re-indexaci\'on de movimientos: \texttt{POST /api/ai/reindex} regenera los vectores con el modelo de \emph{embeddings}.}{capturas_postman/10_re_indexar_movimientos.png}
...
\evidencia{\emph{Dashboard}: balance por moneda, gr\'afico de gastos por categor\'ia, detalle por categor\'ia y \'ultimos movimientos.}{capturas_frontend/01_dashboard.png}
\evidencia{Listado de gastos con filtros por categor\'ia y rango de fechas, y acci\'on de eliminar.}{capturas_frontend/02_gastos.png}
\evidencia{Asistente de IA respondiendo una consulta en lenguaje natural sobre las finanzas del usuario.}{capturas_frontend/03_asistente.png}
...
\evidencia{Consola de AWS (regi\'on \texttt{us-east-1}) con la instancia \texttt{FinZen App} en estado \emph{Running}, tipo \texttt{t3.micro} y su IP el\'astica \texttt{54.198.140.159}.}{capturas_aws/aws_01_instancia.png}
\evidencia{Reglas de \emph{entrada} del Security Group: \texttt{22} (SSH) y \texttt{80} (HTTP), ambas desde \texttt{0.0.0.0/0}.}{capturas_aws/aws_02_security_group.png}
\evidencia{Terminal de la EC2: \texttt{docker compose ps} y \texttt{docker ps} muestran los 6 contenedores en ejecuci\'on, y las pruebas con \texttt{curl} devuelven \texttt{200}.}{capturas_aws/aws_03_contenedores.png}
\evidencia{Navegador accediendo a la aplicaci\'on en \texttt{http://54.198.140.159}: panel de control del usuario autenticado.}{capturas_aws/aws_04_app.png}
\evidencia{Liberaci\'on del puerto \texttt{80} en la EC2: Apache detenido y deshabilitado para que Nginx pueda arrancar.}{capturas_aws/aws_05_apache.png}
...
Los recursos complementarios del proyecto se encuentran en el repositorio de
documentaci\'on \href{https://github.com/UTEC-AII/finzen-docs}{finzen-docs}:
\begin{itemize}[leftmargin=1.2cm]
  \item \textbf{Anexo A} --- Prompt del prototipo:
        \href{https://github.com/UTEC-AII/finzen-docs/blob/main/frontend-prompt/prompt.md}{Link}.
  \item \textbf{Anexo B} --- Colecci\'on Postman y entornos:
        \href{https://github.com/UTEC-AII/finzen-docs/tree/main/postman}{Link}.
  \item \textbf{Anexo C} --- Cat\'alogo completo de APIs:
        \href{https://github.com/UTEC-AII/finzen-docs/blob/main/CATALOGO_DE_APIS.md}{Link}.
  \item \textbf{Anexo D} --- Repositorios de c\'odigo:
        \href{https://github.com/UTEC-AII/finzen-app}{finzen-app},
        \href{https://github.com/UTEC-AII/finzen-webui}{finzen-webui} y
        \href{https://github.com/UTEC-AII/finzen-docs}{finzen-docs}.
  \item \textbf{Anexo E} --- Aplicaci\'on desplegada:
        \href{http://54.198.140.159}{http://54.198.140.159}.
\end{itemize}
